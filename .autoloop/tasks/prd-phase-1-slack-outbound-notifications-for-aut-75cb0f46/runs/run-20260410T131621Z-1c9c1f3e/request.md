...
- `sent_at` MUST be non-null only when `delivery_status = 'sent'`.
- `dead_lettered_at` MUST be non-null only when `delivery_status = 'dead_letter'`.
- `sent_at` and `dead_lettered_at` MUST NOT both be non-null on the same row.
...
- Duplicate handling MUST preserve the reused event's existing target-row state as-is.
- Duplicate handling MUST NOT create, delete, repair, replace, or migrate any `integration_event_targets` row for the reused event, even if `SLACK_DEFAULT_TARGET_NAME`, `SLACK_TARGETS_JSON`, or `SLACK_NOTIFY_*` flags changed after the event was first emitted.
...
- actions that create neither a public message nor a distinct non-initial status transition MUST follow Section 6.3

### 6.3 No-Event Actions

- If an action commits neither an eligible public message for `ticket.public_message_added` nor a distinct non-initial status transition for `ticket.status_changed`, it MUST create no Phase 1 Slack integration rows.
- Specifically, it MUST create no rows in `integration_events`, `integration_event_links`, or `integration_event_targets`.
- In current Stage 1 terms, internal notes and draft rejection follow this rule.
...
Common rules:
- `ticket_url` MUST deep-link an authenticated internal operator to the existing Dev/TI ticket detail route and MUST equal `<normalized APP_BASE_URL>/ops/tickets/<ticket_reference>`.
...
- `<normalized APP_BASE_URL>` means the configured `APP_BASE_URL` with exactly one optional trailing `/` removed before concatenation. `https://autosac.example.local` and `https://autosac.example.local/` MUST therefore produce the same `ticket_url`.
...
- If a later duplicate emission reuses an existing event with zero target rows, duplicate handling MUST leave that event with zero target rows. It MUST NOT create a repair row based on current routing configuration.
...
- if the current `attempt_count` has reached `SLACK_DELIVERY_MAX_ATTEMPTS`, the row MUST transition to `dead_letter` instead of `failed`, using the exact dead-letter field writes below
...
- any transition to `dead_letter`, whether caused by retry exhaustion or terminal failure, MUST set:
  - set `delivery_status = 'dead_letter'`
  - set `dead_lettered_at = now()`
  - keep `sent_at = null`
  - set sanitized `last_error`
  - clear `locked_at` and `locked_by`
  - leave `attempt_count` at its current post-claim value
  - leave `next_attempt_at` unchanged; once a row is terminal, `next_attempt_at` MUST be ignored
...
- every payload builds `ticket_url` as `<normalized APP_BASE_URL>/ops/tickets/<ticket_reference>`, and the result is identical whether `APP_BASE_URL` has a trailing slash or not
- internal notes and draft rejection create no rows in `integration_events`, `integration_event_links`, or `integration_event_targets` in current Stage 1 behavior
...
- duplicate emission attempts preserve the reused event's existing target-row state exactly: they never create a second target row, and they do not repair a previously zero-target event after routing changes
...
- 2xx delivery marks the row `sent`, sets `sent_at`, clears `last_error`, clears the lock, and leaves `dead_lettered_at = null`
...
- rows dead-lettered by retry exhaustion and rows dead-lettered by terminal failure both clear `locked_at` and `locked_by`, preserve the post-claim `attempt_count`, set `dead_lettered_at`, and never schedule another automatic retry
