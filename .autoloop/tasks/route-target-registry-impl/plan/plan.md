...
- Seed `agent_specs/registry.json` with enabled direct-AI route targets that preserve the current persisted taxonomy IDs: `support`, `access_config`, `data_ops`, `bug`, `feature`, and `unknown`.
- Add `manual_review` as the initial human-assist route target with `handler.human_queue_status = "waiting_on_dev_ti"`.
...
- Keep `human_assist + specialist_selection.mode=none` as a supported runtime path even if the initial registry exercises `manual_review` through `mode=auto`; tests must cover `none`, `fixed`, and `auto`.
...
- Implement `shared/routing_registry.py` validation rules from the spec, including enabled/historical semantics and specialist-candidate resolution.
...
- The repository has no DB constraint that hardcodes route-target taxonomy.
