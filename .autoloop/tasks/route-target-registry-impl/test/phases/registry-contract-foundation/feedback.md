...
- TST-001 follow-up | non-blocking | Verified resolved in producer cycle 2. The router, selector, and specialist prompt tests in `tests/test_routing_registry.py`, plus the worker-path prompt test in `tests/test_ai_worker.py`, now assert that legacy placeholder tokens and ticket-class routing phrasing are absent while preserving the intentional compatibility-phase `ticket_class` schema note. Auditor rerun of `.venv/bin/pytest -q tests/test_routing_registry.py tests/test_ai_worker.py tests/test_hardening_validation.py tests/test_foundation_persistence.py` passed with `84 passed`.
