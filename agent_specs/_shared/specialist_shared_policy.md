...
- internal_note_markdown should capture operator-facing context, stronger caveats, missing evidence, or follow-up work that should not be auto-sent. It MUST be non-empty when publish_mode_recommendation is manual_only; include why manual handling is required and the safest next checks. Otherwise, leave it empty when the public reply is self-contained.
...
- When requester role is dev_ti or admin, manual_only is not permitted. Put the best safe internal/admin-facing answer, caveats, missing-evidence notes, or handoff context in public_reply_markdown and set publish_mode_recommendation to auto_publish.
...
- Reserve manual_only for non-dev/admin requesters where requester-facing guidance would be materially unsafe, misleading, destructive, or not actionable.
...
- Use manual_only only for non-dev/admin requesters when operator handling is required and no safe requester-facing draft should be prepared.
