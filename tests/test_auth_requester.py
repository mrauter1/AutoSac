...
def test_requester_list_route_redirects_to_login_without_session(unauth_requester_client):
    response = unauth_requester_client.get("/app", follow_redirects=False)
    assert response.status_code == 303
    assert response.headers["location"] == "/login"


def test_requester_list_route_returns_hx_redirect_for_htmx_without_session(unauth_requester_client):
    response = unauth_requester_client.get("/app", headers={"HX-Request": "true"}, follow_redirects=False)
    assert response.status_code == 200
    assert response.headers["hx-redirect"] == "/login"
    assert "location" not in response.headers


def test_requester_list_route_returns_401_for_json_clients_without_session(unauth_requester_client):
    response = unauth_requester_client.get("/app", headers={"Accept": "application/json"}, follow_redirects=False)
